...
class MemoryManager:
    ...
    def __init__(self):
        ...
        # ------------------------------------------
        # Long-term memory (PostgreSQL) — optional
        # Fails gracefully if PostgreSQL is not running
        # ------------------------------------------
        try:
            from src.memory.postgres_memory import PostgresMemory
            self._pg = PostgresMemory()
        except Exception as e:
            logger.warning(f"MemoryManager: PostgresMemory init failed — {e}")
            self._pg = None
    ...
    def create_session(self, user_id: Optional[str] = None) -> str:
        ...
        # Long-term: create conversation record in PostgreSQL
        if self._pg:
            self._pg.create_conversation(session_id, user_id)
        logger.info(f"MemoryManager: new session created | session_id={session_id}")
        return session_id

    def _create_session_with_id(self, session_id: str) -> None:
        """Create Redis keys for a known session_id (used during PostgreSQL restore)."""
        self._check_client()
        now  = datetime.now(timezone.utc).isoformat()
        meta = {"created_at": now, "last_active": now, "query_count": 0}
        pipe = self.client.pipeline()
        pipe.set(self._HISTORY_PREFIX.format(session_id), json.dumps([]),   ex=self.ttl)
        pipe.set(self._META_PREFIX.format(session_id),    json.dumps(meta), ex=self.ttl)
        pipe.execute()
    ...
    def delete_session(self, session_id: str) -> None:
        ...
        logger.info(f"MemoryManager: session deleted | session_id={session_id}")
    ...
    def get_history(self, session_id: str) -> List[Dict]:
        ...
        If Redis misses (session expired), tries to restore from PostgreSQL.
        ...
        if raw is not None:
            ...
            logger.debug(
                f"MemoryManager: loaded history from Redis | "
                f"session_id={session_id} | turns={len(history)}"
            )
            return history

        # Redis miss — try PostgreSQL restore
        logger.info(
            f"MemoryManager: Redis miss for session_id={session_id} — "
            f"attempting PostgreSQL restore."
        )
        if self._pg:
            restored = self._pg.restore_to_redis(session_id, self)
            if restored:
                logger.info(
                    f"MemoryManager: session restored from PostgreSQL | "
                    f"session_id={session_id} | turns={len(restored)}"
                )
                return restored

        logger.warning(f"MemoryManager: session not found | session_id={session_id}")
        return []
    ...
    def save_history(self, session_id: str, history: List[Dict]) -> None:
        ...
        history_key = self._HISTORY_PREFIX.format(session_id)
        self.client.set(history_key, json.dumps(history), ex=self.ttl)
        logger.debug(
            f"MemoryManager: history saved | session_id={session_id} | turns={len(history)}"
        )
    ...
    def append_turn(
        ...
        # Save back (handles trimming + TTL refresh)
    ...
    def append_exchange(
        ...
        """
        Convenience method — append both user query and
        assistant answer in one call.
        Dual-writes to Redis (short-term) and PostgreSQL (long-term).
        ...
        # Long-term: save both turns to PostgreSQL permanently
        if self._pg:
            self._pg.save_message(session_id, user_id, "user",      user_query)
            self._pg.save_message(session_id, user_id, "assistant", assistant_answer, sources=sources)
    ...
    def get_user_conversations(self, user_id: str, limit: int = 20) -> List[Dict]:
        """Return user's past conversation list from PostgreSQL."""
        if self._pg:
            return self._pg.get_user_conversations(user_id, limit=limit)
        return []
    ...
    def _update_meta(self, session_id: str, role: str) -> None:
        ...
        self.client.set(meta_key, json.dumps(meta), ex=self.ttl)

    def clear_history(self, session_id: str) -> None:
        """Clear chat history for a session but keep the session alive."""
        self._check_client()
        history_key = self._HISTORY_PREFIX.format(session_id)
        self.client.set(history_key, json.dumps([]), ex=self.ttl)
        logger.info(f"MemoryManager: history cleared | session_id={session_id}")
    ...
    def get_recent_history(
        ...
        Args:
            session_id: session identifier
            last_n:     number of recent turns to return (default 4 = 2 exchanges)
        """
        history = self.get_history(session_id)
