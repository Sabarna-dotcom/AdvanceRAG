...
@router.post(
    ...
    Optionally filter by collection ('pdf' or 'audio') and pass chat history
...
async def query(request: QueryRequest) -> QueryResponse:
    ...
    4. Self-reflection + corrective loop (if enabled)
    5. Return structured answer with cited sources
    """
    ...
    manager = GenerationManager(
        use_self_reflection=request.use_self_reflection
    )
    ...
    # Build cited sources response objects
    cited_sources = []
    ...
    # Build reflection response object
    reflection = None
    ...
    if raw_reflection:
        reflection = ReflectionResult(
            overall_confidence=raw_reflection.get(
                "overall_confidence"
            ),
            accuracy_confidence=raw_reflection.get(
                "accuracy_confidence"
            ),
            completeness_confidence=raw_reflection.get(
                "completeness_confidence"
            ),
            citation_confidence=raw_reflection.get(
                "citation_confidence"
            ),
            needs_more_retrieval=raw_reflection.get(
                "needs_more_retrieval"
            ),
            improvement_hint=raw_reflection.get(
                "improvement_hint"
            ),
            uncertainties=raw_reflection.get(
                "uncertainties"
            ),
        )
    ...
    logger.info(
        ...
        f"cited_sources={len(cited_sources)} | "
